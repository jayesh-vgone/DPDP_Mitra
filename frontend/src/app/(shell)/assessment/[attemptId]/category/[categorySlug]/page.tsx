      ...
      <div className="bg-white border-b border-gray-200 px-8 py-5">
        ...
        {detail && (
          <div className="mt-3 flex flex-col gap-2">
            <h2 className="text-xl font-bold text-[#0A0F2C]">{categoryDisplayName}</h2>
      ...
      {detail && !loading && (
        <div className="px-8 py-8 space-y-6 max-w-4xl">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            <p className="text-sm text-gray-700 leading-relaxed">{detail.explanation}</p>
          </div>

          {/* Questions table */}
          <div>
            <h3 className="font-semibold text-[#0A0F2C] text-sm mb-4">
              {t('drillQuestionsTitle', lang)}
            </h3>
            <div className="space-y-3">
              {detail.questions.map((q, i) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-100 p-5">
                  <p className="text-sm font-medium text-[#0A0F2C] leading-snug mb-3">
                    {q.question_text}
                  </p>
                  <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-gray-500">
                    <span className="flex items-center gap-1.5">
                      <span className="font-medium text-gray-700">{t('drillAnswerLabel', lang)}:</span>
                      <AnswerDisplay answerType={q.answer_type} value={q.answer_value} lang={lang} />
                    </span>
                    {q.dpdp_section && (
                      <span className="flex items-center gap-1.5">
                        <Tag size={11} className="text-[#FF9933]" />
                        <span className="font-medium text-gray-700">{t('drillDpdpSection', lang)}:</span>
                        <span className="text-[#FF9933] font-semibold">{q.dpdp_section}</span>
                      </span>
                    )}
                    <span>
                      <span className="font-medium text-gray-700">{t('drillWeight', lang)}:</span>{' '}
                      {q.weight}
                    </span>
                  </div>
                </div>
