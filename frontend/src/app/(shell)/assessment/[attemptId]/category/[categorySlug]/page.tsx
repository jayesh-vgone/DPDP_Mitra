...
function MaturityBadge({ band }: { band: string }) {
  const colors: Record<string, { bg: string; text: string }> = {
    ...
    Moderate: { bg: '#FFF3E0', text: '#FF9933' },
...
function ScoreBar({ score }: { score: number }) {
  const color = score > 70 ? '#138808' : score > 40 ? '#FF9933' : '#DC2626';
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 bg-gray-100 dark:bg-[#1A2756] rounded-full h-3">
        <div
...
function AnswerDisplay({ answerType, value, lang }: { answerType: 'scale' | 'boolean'; value: number; lang: 'en' | 'hi' }) {
  ...
  const textColor = value >= 3 ? '#138808' : value >= 2 ? '#FF9933' : '#DC2626';
...
export default function CategoryDrillPage() {
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#F9FAFB] dark:bg-[#0A0F2C]">
      {/* Header */}
      <div className="bg-white dark:bg-[#0F1A3E] border-b border-gray-200 dark:border-[#1A2756] px-8 py-5">
        <Link href="/dashboard" className="text-xs font-medium text-[#FF9933] hover:underline">
          {t('drillBackToDashboard', lang)}
        </Link>
        {detail && (
          <div className="mt-3 flex flex-col gap-2">
            <h2 className="text-xl font-bold text-[#0A0F2C] dark:text-gray-100">{categoryDisplayName}</h2>
      ...
      {detail && !loading && (
        <div className="px-8 py-8 space-y-6 max-w-4xl">
          ...
          {/* Questions table */}
          <div>
            <h3 className="font-semibold text-[#0A0F2C] dark:text-gray-100 text-sm mb-4">
              {t('drillQuestionsTitle', lang)}
            </h3>
            <div className="space-y-3">
              {detail.questions.map((q, i) => (
                <div key={i} className="bg-white dark:bg-[#0F1A3E] rounded-2xl border border-gray-100 dark:border-[#1A2756] p-5">
                  <p className="text-sm font-medium text-[#0A0F2C] dark:text-gray-100 leading-snug mb-3">
                    {q.question_text}
                  </p>
                  <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {q.dpdp_section && (
                      <span className="flex items-center gap-1.5">
                        <Tag size={11} className="text-[#FF9933]" />
                        <span className="font-medium text-gray-700 dark:text-gray-300">{t('drillDpdpSection', lang)}:</span>
                        <span className="text-[#FF9933] font-semibold">{q.dpdp_section}</span>
                      </span>
                    )}
                    <span>
